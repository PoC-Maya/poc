...
export async function createAvailabilityMarketplaceExperience(prevState, formData) {
  try {
    ...
    return { 
      success: true,
      message: "createAvailabilityTemplate executado com sucesso",
      // Dados adicionais que você queira retornar
    };

  } catch (error) {
    console.error("createAvailabilityTemplate error:", error);
    return {
      success: false,
      errors: {
        _form: "Erro ao executar createAvailabilityTemplate. Tente novamente.",
      },
    };
  }
}
