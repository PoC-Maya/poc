...
export async function uploadCertifications(prevState, formData) {
  try {
    ...
    return { 
      success: true,
      message: "uploadCertifications executado com sucesso",
      // Dados adicionais que você queira retornar
    };

  } catch (error) {
    console.error("uploadCertifications error:", error);
    return {
      success: false,
      errors: {
        _form: "Erro ao executar uploadCertifications. Tente novamente.",
      },
    };
  }
}
