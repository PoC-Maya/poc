...
export async function verifyGuide(prevState, formData) {
  try {
    ...
    return { 
      success: true,
      message: "verifyGuide executado com sucesso",
      // Dados adicionais que você queira retornar
    };

  } catch (error) {
    console.error("verifyGuide error:", error);
    return {
      success: false,
      errors: {
        _form: "Erro ao executar verifyGuide. Tente novamente.",
      },
    };
  }
}
