...
/**
* @description Update an existing guide experience
...
*   
*   // Basic Information
*   "title": "Updated Boat Tour in Cancun",
*   "description": "An updated amazing boat tour through the crystal clear waters of Cancun",
...
const schema = z.object({
  ...
  message: "Minimum participants must be less than or equal to maximum participants",
  ...
  message: "The sum of maximum adults, teens, and children must be between minimum and maximum participants",
  path: ["max_adults"]
}).refine(data => {
  ...
  message: "Price tiers must cover the entire range of participants without gaps",
  path: ["price_tiers"]
});
...
export async function updateExperience(prevState, formData) {
  try {
    ...
    const jsonFields = [
      'itinerary', 'price_tiers', 'gallery_images', 'working_hours', 'tag_ids'
    ];
    ...
    if (profile.user_type !== 'guide') {
      return {
        success: false,
        errors: {
          _form: "Only guides can update experiences",
        },
      };
    }
    
    try {
      // Call the stored procedure to update the experience
      const { data: result, error } = await supabase.rpc('update_guide_experience', {
        p_guide_id: user.id,
      ...
      // The result is the ID of the updated experience
      const experienceId = result;
      
      // Revalidate paths to update UI data
      revalidatePath('/guide/experiences');
      revalidatePath(`/guide/experiences/${experienceId}`);
      revalidatePath('/experiences');
      
      return { 
        success: true,
        message: "Experience updated successfully and sent for review!",
        data: {
          id: experienceId
        }
      };
      ...
      return {
        success: false,
        errors: {
          _form: error.message || "Error updating experience. Please try again.",
        },
      };
    }
  } catch (error) {
    ...
    return {
      success: false,
      errors: {
        _form: "Error updating experience. Please try again.",
      },
    };
  }
}