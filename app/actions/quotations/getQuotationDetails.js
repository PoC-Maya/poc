...
export async function getQuotationDetails(prevState, formData) {
  try {
    ...
    return { 
      success: true,
      message: "getQuotationDetails executado com sucesso",
      // Dados adicionais que você queira retornar
    };

  } catch (error) {
    console.error("getQuotationDetails error:", error);
    return {
      success: false,
      errors: {
        _form: "Erro ao executar getQuotationDetails. Tente novamente.",
      },
    };
  }
}
